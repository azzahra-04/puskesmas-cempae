<script>
  ...
  let searchValue = "";
  let filteredData = data.facilities;

  function handleSearch() {
    let filter = searchValue.toLowerCase();
    filteredData = data.facilities.filter((facility) => {
      return facility.name.toLowerCase().includes(filter);
    });
  }
...
<div class="max-w-screen-xl mx-auto px-4 md:px-8">
  <div class="pt-10 items-start justify-between py-4 border-b md:flex">
    <div>
      <h3 class="text-2xl font-bold">Kelola Fasilitas</h3>
    </div>
    ...
    <table class="w-full table-fixed text-sm text-left">
      <thead class="bg-dark-green font-medium border-b">
        <tr>
          ...
          <th class="py-3 px-6 border">Nama Fasilitas</th>
      ...
      <tbody>
        {#if filteredData.length > 0}
          {#each filteredData as { _id: id, name, image, description }}
            <tr class="border">
              <td class="px-6 py-4">
                <img src={image} alt="" class="w-10 h-10 rounded" /></td
              >
              ...
              <td class="px-6 py-4 border">
                <div class="flex items-center justify-center">
                  <a
                    href="/admin-pkm/dashboard/facilities/{id}"
                    class="py-2 px-3 mr-4 font-medium text-indigo-600 hover:bg-dark-green rounded"
                  >
