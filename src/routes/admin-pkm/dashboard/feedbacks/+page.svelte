...
<div class="max-w-screen-xl mx-auto px-4 md:px-8">
  <div class="pt-10 items-start justify-between py-4 border-b md:flex">
    <h3 class="text-2xl font-bold">Laporan Pengguna Sistem</h3>
  </div>
</div>
<div class="max-w-screen-xl mx-auto px-4 md:px-8">
    ...
    <table class="w-full table-fixed text-sm text-left">
      <thead class="bg-dark-green font-medium border-b">
        <tr>
          ...
          <th class="py-3 px-6 border">Pesan</th>
          <th class="py-3 px-6 border"></th>
        </tr>
      </thead>
      <tbody>
        {#each data.feedbacks as { _id: id, name, email, message }}
          <tr class="border">
            <td class="px-6 py-4 border">
              <span class="block text-sm font-medium">{name}</span>
            </td>
            <td class="px-6 py-4 border truncate w-32">{email}</td>
            <td class="px-6 py-4 border truncate w-32">{message}</td>
            <td class="px-6 py-4 border">
              <button
                on:click={() => {
